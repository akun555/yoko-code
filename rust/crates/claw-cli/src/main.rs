...
fn render_cli_error(problem: &str) -> String {
    ...
    lines.push("  Help             yoko --help".to_string());
    lines.join("\n")
}
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "-p" => {
                // YOKO Code compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // YOKO Code compat: --print makes output non-interactive
...
fn format_direct_slash_command_error(command: &str, is_unknown: bool) -> String {
    ...
    if is_unknown {
        ...
        lines.push("  Try              Start `yoko` to use interactive slash commands".to_string());
        lines.push(
            "  Tip              Resume-safe commands also work with `yoko --resume SESSION.json ...`"
                .to_string(),
        );
    }
    lines.join("\n")
...
fn print_bootstrap_plan() {
    for phase in runtime::BootstrapPlan::yoko_default().phases() {
        println!("- {phase:?}");
    }
}
...
fn default_oauth_config() -> OAuthConfig {
    OAuthConfig {
        ...
        authorize_url: String::from("https://platform.yoko.dev/oauth/authorize"),
        token_url: String::from("https://platform.yoko.dev/v1/oauth/token"),
        callback_port: None,
        manual_redirect_url: None,
        scopes: vec![
            String::from("user:profile"),
            String::from("user:inference"),
            String::from("user:sessions:yoko_code"),
        ],
    }
}
...
impl LiveCli {
    ...
    fn startup_banner(&self) -> String {
        ...
        let has_yoko_md = cwd
            .as_ref()
            .is_some_and(|path| path.join("YOKO.md").is_file());
        let mut lines = vec![
            format!(
                "{} {}",
                if color {
                    "\x1b[1;38;5;45m🦞 YOKO Code\x1b[0m"
                } else {
                    "YOKO Code"
                },
                if color {
        ...
        if !has_yoko_md {
            lines.push(
                "  First run        /init scaffolds YOKO.md, .yoko.json, and local session files"
                    .to_string(),
            );
        }
        lines.join("\n")
    ...
    fn run_commit(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let path = write_temp_text_file("yoko-commit-message.txt", &message)?;
    ...
    fn run_pr(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("yoko-pr-body.md", &body)?;
    ...
    fn run_issue(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("yoko-issue-body.md", &body)?;
            let output = Command::new("gh")
...
fn sessions_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let path = cwd.join(".yoko").join("sessions");
...
fn render_memory_report() -> Result<String, Box<dyn std::error::Error>> {
  ...
  Instruction files {}",
    ...
    if project_context.instruction_files.is_empty() {
        ...
        lines.push(
            "  No YOKO instruction files discovered in the current directory ancestry.".to_string(),
        );
    } else {
        lines.push("Discovered files".to_string());
...
fn init_yoko_md() -> Result<String, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    Ok(initialize_repo(&cwd)?.render())
}

fn run_init() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", init_yoko_md()?);
    Ok(())
}
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "YOKO Code CLI v{VERSION}")?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  yoko                                  Start the interactive REPL"
    )?;
    writeln!(
        out,
        "  yoko \"summarize this repo\"            Run one prompt and exit"
    )?;
    writeln!(
        out,
        "  yoko prompt \"explain src/main.rs\"     Explicit one-shot prompt"
    )?;
    writeln!(
        out,
        "  yoko --resume SESSION.json /status    Inspect a saved session"
    )?;
    writeln!(out)?;
    ...
    writeln!(
        out,
        "  yoko dump-manifests                   Read upstream TS sources and print extracted counts"
    )?;
    writeln!(
        out,
        "  yoko bootstrap-plan                   Print the bootstrap phase skeleton"
    )?;
    writeln!(
        out,
        "  yoko agents                           List configured agents"
    )?;
    writeln!(
        out,
        "  yoko skills                           List installed skills"
    )?;
    writeln!(out, "  yoko system-prompt [--cwd PATH] [--date YYYY-MM-DD]")?;
    writeln!(
        out,
        "  yoko login                            Start the OAuth login flow"
    )?;
    writeln!(
        out,
        "  yoko logout                           Clear saved OAuth credentials"
    )?;
    writeln!(
        out,
        "  yoko init                             Scaffold YOKO.md + local files"
    )?;
    writeln!(out)?;
    writeln!(out, "Flags")?;
    writeln!(
    ...
    writeln!(out, "  yoko --model opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  yoko --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(
        out,
        "  yoko --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(
        out,
        "  yoko --resume session.json /status /diff /export notes.txt"
    )?;
    writeln!(out, "  yoko agents")?;
    writeln!(out, "  yoko /skills")?;
    writeln!(out, "  yoko login")?;
    writeln!(out, "  yoko init")?;
    Ok(())
}
...
mod tests {
    ...
    fn shared_help_uses_resume_annotation_copy() {
        ...
        assert!(help.contains("available via yoko --resume SESSION.json"));
    }
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("yoko init"));
        assert!(help.contains("yoko agents"));
        assert!(help.contains("yoko skills"));
        assert!(help.contains("yoko /skills"));
    }
 ...
 M src/main.rs",
    ...
    fn init_template_mentions_detected_rust_workspace() {
        let rendered = crate::init::render_init_yoko_md(std::path::Path::new("."));
        assert!(rendered.contains("# YOKO.md"));
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/yoko-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p yoko-cli"}"#)
                .contains("cargo test -p yoko-cli")
        );
        assert_eq!(
