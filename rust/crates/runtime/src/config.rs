...
pub const YOKO_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".yoko.json"),
            |parent| parent.join(".yoko.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".yoko.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".yoko").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".yoko").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("YOKO_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".yoko")))
        .unwrap_or_else(|| PathBuf::from(".yoko"))
}
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".yoko.json");
...
mod tests {
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".yoko");
    ...
    #[test]
    fn loads_and_merges_yoko_code_config_files_by_precedence() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".yoko");
        fs::create_dir_all(cwd.join(".yoko")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".yoko.json"),
        ...
        assert_eq!(YOKO_SETTINGS_SCHEMA_NAME, "SettingsSchema");
