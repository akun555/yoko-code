...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("YOKO.md"),
            dir.join("YOKO.local.md"),
            dir.join(".yoko").join("YOKO.md"),
            dir.join(".yoko").join("instructions.md"),
        ] {
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No YOKO Code settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".yoko")).expect("nested yoko dir");
        fs::write(root.join("YOKO.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("YOKO.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".yoko")).expect("apps yoko dir");
        fs::write(root.join("apps").join("YOKO.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".yoko").join("instructions.md"),
            "apps dot yoko instructions",
        )
        .expect("write apps dot yoko instructions");
        fs::write(nested.join(".yoko").join("YOKO.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".yoko").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("YOKO.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("YOKO.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.yoko/YOKO.md")),
            "YOKO.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("YOKO.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? YOKO.md"));
    ...
    #[test]
    fn load_system_prompt_reads_yoko_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".yoko")).expect("yoko dir");
        fs::write(root.join("YOKO.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".yoko").join("settings.json"),
        ...
        let original_yoko_home = std::env::var("YOKO_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("YOKO_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_yoko_home {
            std::env::set_var("YOKO_CONFIG_HOME", value);
        } else {
            std::env::remove_var("YOKO_CONFIG_HOME");
        }
    ...
    #[test]
    fn renders_yoko_code_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".yoko")).expect("yoko dir");
        fs::write(root.join("YOKO.md"), "Project rules").expect("write YOKO.md");
        fs::write(
            root.join(".yoko").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
    ...
    #[test]
    fn discovers_dot_yoko_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".yoko")).expect("nested yoko dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".yoko/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/YOKO.md"),
