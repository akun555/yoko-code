...
const STARTER_YOKO_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# YOKO Code local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".yoko/settings.local.json", ".yoko/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let yoko_dir = cwd.join(".yoko");
    artifacts.push(InitArtifact {
        name: ".yoko/",
        status: ensure_dir(&yoko_dir)?,
    });

    let yoko_json = cwd.join(".yoko.json");
    artifacts.push(InitArtifact {
        name: ".yoko.json",
        status: write_file_if_missing(&yoko_json, STARTER_YOKO_JSON)?,
    });
    ...
    let yoko_md = cwd.join("YOKO.md");
    let content = render_init_yoko_md(cwd);
    artifacts.push(InitArtifact {
        name: "YOKO.md",
        status: write_file_if_missing(&yoko_md, &content)?,
    });
...
pub(crate) fn render_init_yoko_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# YOKO.md".to_string(),
        String::new(),
        "This file provides guidance to YOKO Code (yokocode.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.yoko.json`; reserve `.yoko/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `YOKO.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_yoko_md};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("yoko-init-{nanos}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".yoko/           created"));
        assert!(rendered.contains(".yoko.json       created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("YOKO.md          created"));
        assert!(root.join(".yoko").is_dir());
        assert!(root.join(".yoko.json").is_file());
        assert!(root.join("YOKO.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".yoko.json")).expect("read yoko json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".yoko/settings.local.json"));
        assert!(gitignore.contains(".yoko/sessions/"));
        let yoko_md = fs::read_to_string(root.join("YOKO.md")).expect("read yoko md");
        assert!(yoko_md.contains("Languages: Rust."));
        assert!(yoko_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("YOKO.md"), "custom guidance\n").expect("write existing yoko md");
        fs::write(root.join(".gitignore"), ".yoko/settings.local.json\n").expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first
            .render()
            .contains("YOKO.md          skipped (already exists)"));
        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".yoko/           skipped (already exists)"));
        assert!(second_rendered.contains(".yoko.json       skipped (already exists)"));
        assert!(second_rendered.contains(".gitignore       skipped (already exists)"));
        assert!(second_rendered.contains("YOKO.md          skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(root.join("YOKO.md")).expect("read existing yoko md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".yoko/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".yoko/sessions/").count(), 1);
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_yoko_md(Path::new(&root));
