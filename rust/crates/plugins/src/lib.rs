...
const MANIFEST_RELATIVE_PATH: &str = ".yoko-plugin/plugin.json";
...
impl PluginTool {
    ...
    pub fn execute(&self, input: &Value) -> Result<String, PluginError> {
        ...
        process
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .env("YOKO_PLUGIN_ID", &self.plugin_id)
            .env("YOKO_PLUGIN_NAME", &self.plugin_name)
            .env("YOKO_TOOL_NAME", &self.definition.name)
            .env("YOKO_TOOL_INPUT", &input_json);
        if let Some(root) = &self.root {
            process
                .current_dir(root)
                .env("YOKO_PLUGIN_ROOT", root.display().to_string());
        }
...
mod tests {
  ...
  "commands": [
    ...
    fn write_tool_plugin_with_name(root: &Path, name: &str, version: &str, tool_name: &str) {
        ...
        write_file(
            &script_path,
            "#!/bin/sh\nINPUT=$(cat)\nprintf '{\"plugin\":\"%s\",\"tool\":\"%s\",\"input\":%s}\\n' \"$YOKO_PLUGIN_ID\" \"$YOKO_TOOL_NAME\" \"$INPUT\"\n",
        );
        #[cfg(unix)]
        {
